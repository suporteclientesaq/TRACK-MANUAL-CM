/**
 * Tabelas do modo online (Postgres). Usado de dois jeitos:
 *  - conexão direta (DATABASE_URL): o painel executa isto sozinho na primeira conexão;
 *  - API do Supabase (extensão do Netlify): a pessoa cola isto uma vez no SQL Editor.
...
/** Versão para colar no SQL Editor do Supabase (avisa a API para recarregar as tabelas). */
export const SUPABASE_SETUP_SQL = `-- Track Manual: cria as tabelas do painel. Pode rodar mais de uma vez.
${SCHEMA_SQL.trim()}
